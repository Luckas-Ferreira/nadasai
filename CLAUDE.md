...
- **`tool-content.ts` coverage is deliberately partial.** Four *specific* answers per tool is the point; four generic privacy answers repeated 31 times is thin duplicate content, which Google treats accordingly. A tool with no entry falls back to the dictionary's generic set, and `jsonld.spec.ts` reports which tools are still uncovered rather than letting the gap pass unnoticed.
- **`sitemap.spec.ts` fetches `public/sitemap.xml`** (the Karma target serves `public/` as assets) and fails if a tool URL is missing. The sitemap is still hand-maintained — there is no generator — so this is the cheap half of the fix; a `prebuild` generator driven by the same `TOOLS` array is the durable one.
