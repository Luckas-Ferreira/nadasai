...
Eighteen tools in three modules, all declared in `core/tools/tools.ts` (`category: 'image' | 'pdf' | 'audio'`):
...
### Audio (`core/audio/`, `features/cut-audio/`)

One tool so far, and it takes **no dependency at all** — the Web Audio API decodes, and everything else is arithmetic over Float32 in `core/audio/`. Same split as the image pipeline: pure and unit-tested in `core/`, UI state in the component, one stateless service doing the work.
